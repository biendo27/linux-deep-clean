...
func TestRecoveryLedgerPublicAdapterPropagatesSessionFailuresWithoutCreatingFacts(t *testing.T) {
	...
	t.Run("list failure", func(t *testing.T) {
		...
		if recovery, err := ledger.Reserve(ctx, action, planDigest); !errors.Is(err, injected) || recovery.Token() != "" {
...
func TestRecoveryLedgerPublicAdapterRejectsMissingAndIllegalTransitionIdentities(t *testing.T) {
	...
	reserved, err := ledger.Reserve(ctx, testRecoveryAction(t, domain.ActionTrashPath), testRecoveryPlanDigest(t))
	if err != nil {
		t.Fatalf("Reserve() error = %v", err)
	}
...
func TestRecoveryLedgerPublicReserveBlocksAtExactRetainedRecordCapacity(t *testing.T) {
	...
	for index := 0; index < maximumRecoveryLedgerRecords; index++ {
		...
		binding, err := newRecoveryBinding(action, planDigest, fmt.Sprintf("ldc-%064x", index+1))
		if err != nil {
	...
	if _, err := ledger.Reserve(ctx, testRecoveryActionAtPath(t, domain.ActionQuarantinePath, [][]byte{[]byte("cache"), []byte("next")}), planDigest); !errors.Is(err, ErrRecoveryConflict) {
